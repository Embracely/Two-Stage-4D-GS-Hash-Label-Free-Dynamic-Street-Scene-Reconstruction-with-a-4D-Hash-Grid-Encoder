...
class Grid:
    ...
    用于表示3D网格的类
    """
    def __init__(self, aabb, res):
        """
        初始化Grid
        aabb: 轴对齐包围盒 [min_point, max_point]
        res: 分辨率 [x_res, y_res, z_res]
        """
        self.aabb = aabb  # [min_point, max_point]
        self.res = res    # [x_res, y_res, z_res]
        self.voxel_size = (aabb[1] - aabb[0]) / torch.tensor(res, device=aabb.device)
        
    def is_in_aabb(self, xyz):
        """
        检查点是否在AABB内
        xyz: 点坐标 [N, 3]
        返回: 布尔掩码 [N]
        """
        return torch.all((xyz >= self.aabb[0]) & (xyz <= self.aabb[1]), dim=-1)
    
    def get_grid_and_voxel_coords(self, xyz):
        """
        获取点的网格索引和体素内归一化坐标
        xyz: 点坐标 [N, 3]
        返回: grid_xyz [N, 3], voxel_xyz [N, 3]
        """
        # 计算网格索引
        grid_xyz = torch.floor((xyz - self.aabb[0]) / self.voxel_size).long()
        # 确保索引在有效范围内
        grid_xyz = torch.clamp(grid_xyz, min=0, max=torch.tensor(self.res, device=xyz.device)-1)
        
        # 计算体素内归一化坐标 [0,1]
        min_corner = self.aabb[0] + grid_xyz * self.voxel_size
        voxel_xyz = (xyz - min_corner) / self.voxel_size
        
        return grid_xyz, voxel_xyz
    
    def normVoxel_to_global(self, voxel_xyz, grid_xyz):
        """
        将体素内归一化坐标转换为全局坐标
        voxel_xyz: 体素内归一化坐标 [N, 3]
        grid_xyz: 网格索引 [N, 3]
        返回: 全局坐标 [N, 3]
        """
        min_corner = self.aabb[0] + grid_xyz * self.voxel_size
        global_xyz = min_corner + voxel_xyz * self.voxel_size
        return global_xyz
    
    def norm_scale_by_voxel_size(self, scale, scale_ratio=1.0):
        """
        根据体素大小归一化缩放参数
        scale: 缩放参数 [N, 3]
        scale_ratio: 缩放比例
        返回: 归一化后的缩放参数 [N, 3]
        """
        return scale / (self.voxel_size * scale_ratio)
...
class GridGaussianModel(GaussianModel):
    ...
    """
    def __init__(self, sh_degree, grid: Grid=None, args=None) -> None:
