...
class GaussianModel:
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        max_prune_percent = 0.05  # 最大允许修剪5%的点
        
        if points_to_prune > total_points * max_prune_percent and total_points > 10000:
            # 如果要修剪的点超过阈值，随机选择一部分点进行修剪
            max_points_to_prune = int(total_points * max_prune_percent)
            prune_indices = torch.where(prune_mask)[0]
            
            if len(prune_indices) > max_points_to_prune:
                # 随机选择点进行修剪
                perm = torch.randperm(len(prune_indices))
                selected_indices = prune_indices[perm[:max_points_to_prune]]
                
                # 创建新的掩码
                new_prune_mask = torch.zeros_like(prune_mask)
                new_prune_mask[selected_indices] = True
                prune_mask = new_prune_mask
                
                print(f"[PRUNING SAFETY] Limited pruning from {points_to_prune} to {max_points_to_prune} points ({max_prune_percent*100:.1f}% of total)")
